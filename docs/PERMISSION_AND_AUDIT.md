...
- [七、文件 / 配置布局](#七文件--配置布局)
- [八、跨平台编译说明](#八跨平台编译说明)
- [九、对接消费方（如 Acme 平台）](#九对接消费方如-acme-平台)
...
1. **systray 菜单**：状态栏图标 🛡 Acme、"授权管理…"、"暂停 1 小时"、"恢复"、"退出"
...
✅ **AI 误读员工敏感文件**（要弹同意，员工可拒绝）
✅ **AI 写明显危险命令**（`scp` / `rsync` / `crontab` 等命令策略 token-level 拦截）
✅ **AI 静默开 PTY**（每次开都要员工同意）
✅ **AI 越界访问 hardlock 路径**（`~/.ssh`、`~/.aws` 等默认禁，且 GUI 不能解锁）
✅ **运维盲区**（每条决策都进审计，30s 内推到中央）
...
```bash
# 在 Acme UI 创建 enrollment → 拷出 install 命令
curl -fsSL <PLATFORM_BASE>/api/agent-system/sandboxes/install/sandrpod-agent.sh \
  | PLATFORM_ENROLL='<jwt>' bash

# 期望日志：
#   ✓ launchd 已注册: ~/Library/LaunchAgents/com.example.sandbox.plist
#   ✓ sandrpod-tray 已注册并启动（菜单栏将出现 🛡 Acme 图标）
#   Permission : prompt
...
#   5. 30 秒内 Acme UI "AI 决策审计" Tab 出现两条 audit
```
