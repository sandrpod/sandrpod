...
# ========== Connect Command (toC agent) ==========

@cli.command()
@click.argument("name")
@click.option("--token", default=None, envvar="SANDRPOD_TOKEN", help="API token")
@click.option("--work-dir", default=None, help="Working directory for code execution")
@click.option("--reconnect", default=5, help="Reconnect delay in seconds")
@click.pass_context
def connect(ctx, name, token, work_dir, reconnect):
    """Connect this machine as a local sandbox agent.

    NAME is the sandbox name that will appear in the API server's sandbox list.

    Requires the sandrpod-agent binary to be installed. If it's not on PATH,
    set SANDRPOD_AGENT_BIN to the full path.

    Example:
        sandrpod-cli connect my-laptop --token my-secret-token
    """
    import subprocess
    import shutil

    api_url = ctx.obj["client"].api_url

    # Locate the agent binary
    agent_bin = os.environ.get("SANDRPOD_AGENT_BIN") or shutil.which("sandrpod-agent")
    if not agent_bin:
        click.echo(
            "Error: sandrpod-agent binary not found.\n"
            "Build it with: go build -o sandrpod-agent ./cmd/agent\n"
            "Then add it to PATH or set SANDRPOD_AGENT_BIN.",
            err=True,
        )
        sys.exit(1)
    ...
    click.echo(f"Starting agent as sandbox {name!r} → {api_url}")
    ...
    try:
        ...
        click.echo(f"Error: could not execute {agent_bin}", err=True)
        sys.exit(1)
